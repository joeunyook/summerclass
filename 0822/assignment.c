...
void hanoi_tower_move_n(hanoi_tower_t* ht, unsigned int src, unsigned int dst, unsigned int n)
{
    unsigned int remain = 3 - src -dst;
    if (n==1)
    {
        hanoi_tower_move(ht, src, dst);
        return;
    }
    hanoi_tower_move_n(ht, src, remain, n-1);
    hanoi_tower_move(ht, src, dst);
    hanoi_tower_move_n(ht, remain, dst, n-1);
}
...
int main(void)
{
    ...
    hanoi_tower_move_n(&tower, 0, 2, 3);
    return 0;
}